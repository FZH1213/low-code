...
import { Button, message, Modal, Form, Input, Spin, Popconfirm, Tree, Select, TreeSelect } from 'antd';

import { PlusCircleOutlined, CalendarOutlined, RestFilled } from '@ant-design/icons';

import { getDataList, add, execByCode } from './service';
...
const LeftDataList = React.forwardRef((props, ref) => {
  ...
  const getAPiGroup = async () => {
    const res = await execByCode(JSON.stringify({}), "biz.system.api.select")
    if (res.response.code == 0) {
      let arr = [];
      let arrs = []
      ...
      if (res.response.data.length > 0) {
        ...
        // arr = arr.concat([...res.response.data])
        arrs = toTree([...res.response.data])
        console.log("wqeweqweewq", [...res.response.data])
        console.log("arrs ==>", arrs)
      }
      // setPidOptions(arr)
      setPidOptions1(arrs)
      // props.getAPiSelect(arr)
      props.getAPiSelect1(arrs)
      // console.log("setPageSelect ==>", pidOptions)
    }
  }
  ...
  const handleAddSubmit = () => {
    ...
    if (
      ...
      addFormRef.current.validateFields().then((formData) => {
        ...
        add({ ...obj, paramsJson: '', ruleJson: '', contentJson: '' }).then((res) => {
          if (res.code === 0) {
          ...
          >
            ...
            <Form ref={addFormRef}>
              <Form.Item
                ...
                wrapperCol={{ span: 18 }}
              >
                <TreeSelect value={value} showSearch treeData={pidOptions1} onChange={onChange} allowClear
              ...
              >
                ...
                >
                  ...
                  {/* <Option key="0" value="新增" >新增</Option>
                  <Option key="1" value="修改" >修改</Option>
                  <Option key="2" value="查询" >查询</Option>
                  <Option key="3" value="删除" >删除</Option> */}
                </Select>
              </Form.Item>
              {/* <Form.Item
                name="contentJson"
      ...
      >
        ...
        <div className={styles.left_wrapper} style={{ overflow: "auto" }}>
          <Tree
            ...
            expandedKeys={expressKeys} treeData={renderListData} titleRender={(node) => {
              return (
                <>
                  {searchValue != null && searchValue.length != null && !!searchValue.length ? (
                    <>
                      {node.children.length != 0 && <div>{getSearchRenderItem(node.title, node)}</div>}
                      {node.children.length == 0 && <div>{getSearchRenderItem(`${node.title}（${node.value}）`, node)}</div>}
                    </>
                  ) : (
                    <>
                      {node.children.length != 0 && <div>{getSearchRenderItem(node.title, node)}</div>}
