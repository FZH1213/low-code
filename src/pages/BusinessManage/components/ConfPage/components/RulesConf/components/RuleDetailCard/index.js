...
const ParamsDetailCard = (props) => {
  ...
  const onTreeSelectChange = (value, label, extra) =>{
    let objBusi = extra.triggerNode.props.objBusi;
    console.log('objBusi.reqParam',objBusi.reqParam);
    if (objBusi.reqParam) {
      formRef.current.setFieldsValue({
        params:objBusi.reqParam
      })
    }else{
      formRef.current.setFieldsValue({
        params:'{}'
        ...
        >
          ...
          <Form ref={formRef}>
            <div>
              ...
              >
                <Form.Item labelCol={{ span: 5 }} name={`resultName`} label="返回结果">
                    <Input disabled={editing === false ? true : false}></Input>
                  </Form.Item>
              </div>
            </div>
