...
import { editTable, getDataDdlColumn, addDdl } from './service';
...
const EditTableModal = (props) => {
  ...
  const handleSubmit = () => {
    formRef.current.validateFields().then((value) => {
      ...
      editTable({
        ...
        listData: tableRow,
        tableId: props.tableFormData.id,
      }).then((res) => {
        if (res.code === 0) {
          // 刷新列表
  ...
  useEffect(() => {
    const params = {
      tableId: props.tableFormData.id,
      sourceId: props.tableFormData.dataSourceId,
    };
    getDataDdlColumn(params).then((res) => {
      if (res.code === 0) {
        setTableRow(res.data);
      }
    });
  }, [props.tableFormData.id]);
  ...
  const handleaddDdl = (id, sourceId) => {
    const params = {
      id,
      sourceId,
    };
    addDdl(params).then((res) => {
      if (res.code === 0) {
        // 刷新列表
        props.refreshTable && props.refreshTable();
        // 关闭弹框
  ...
  return (
    ...
    >
      {/* form={form} */}
      <Form ref={formRef} initialValues={props.tableFormData}>
        <Form.Item
          name="tableName"
          ...
          colon={false}
        >
          <Input disabled value={props.tableFormData.tableName}></Input>
        </Form.Item>
        {/* <Form.Item
                name="createTable"
              ...
              >
                <Input.TextArea
                  ...
                  defaultValue={props.tableFormData.createTable}
                  value={props.tableFormData.createTable}
                ></Input.TextArea>
              </Form.Item> */}
      </Form>
      <div
      ...
      >
        ...
        {props.tableFormData.state === 2 && (
          <Button
            ...
            onClick={() => {
              handleaddDdl(props.tableFormData.id, props.tableFormData.dataSourceId);
            }}
          >
            同步到库表
          </Button>
