...
import { ACCESS_TOKEN_KEY, REPLACE_FILE_PATH_STR } from '@/utils/constant';
...
const RichText = (props: any) => {
  ...
  const getNewFilePath = async () => {
    ...
    if (res0.response.code === 0 && !!res0.response.data) {
      if (!!props.values) {
        let arr = props.values.replaceAll(REPLACE_FILE_PATH_STR, res0.response.data)
  ...
  const handleChange = (editorState: any) => {
    ...
    if (!!filePath) {
      //替换文件地址
      let newStr = editorState.toRAW().replaceAll(filePath, REPLACE_FILE_PATH_STR);
