      ...
      >
        {type === 'select' ? (
          ...
          >
            {list.length &&
              list.map((item: any) =>
                item.parmDesc == '是' || item.parmDesc == '否' ? (
                  <Option value={item.parmVal}>{item.parmDesc}</Option>
                ) : (
                  <Option value={item.parmVal}>{item.parmVal}</Option>
                ),
              )}
          </Select>
        ) : (
...
class ModalTableEdit extends React.Component<EditableTableProps, EditableTableState> {
  ...
  render() {
    ...
    const column = columns.map((col: any) => {
      ...
      if (col.key == 'isDisabled' || col.key == 'isId' || col.key == 'isFilter') {
