...
const ModalFormItem: React.FC<{}> = (props: any) => {
  ...
  const intCode = props.intCode ? props.intCode : false;
  ...
  //暂时只改了return里面的item.filterType  item.componentCode.valueType
  return (
    <Modal
                    ...
                    props.formFieldsProp.length > 0 ? props.formFieldsProp.filter((it: any) => it.isDisabled != 1).map((item: any, i: any) => (
                      <Col
                        ...
                        <Form.Item {...item.formItemProps} name={item.tableColum} label={item.displayName}
                          ...
                          initialValue={(props.recordDetial && props.recordDetial[item.tableColum]) ? props.recordDetial[item.tableColum] : item.componentCode.valueType == "datePicker" ? moment() : item.componentCode.valueType == "select" && !item?.code && !!props.initData && props.initData[item.tableColum] && props.initData[item.tableColum].length ? props.initData[item.tableColum][0][item.componentCode.fieldNames.value] : undefined}
                        >
                          {
                            item.componentCode.valueType == "InputNumber" ?
                              <InputNumber
                                style={{ width: '100%' }}
                                {...item.componentCode}
                              ...
                              />
                              :
                              item.componentCode.valueType == "select" ?
                                (
                                  <Select
                                    style={{ width: '100%' }}
                                    {...item.componentCode}
                                  ...
                                  item.componentCode.valueType == "datePicker" ?
                                    (
                                    ...
                                    (
                                      item.componentCode.valueType == "rangerPicker" ?
                                        (
                                          <RangePicker
                                            {...item.componentCode}
                                            getPopupContainer={(triggerNode: any) => triggerNode.parentNode}
                                          ...
                                          item.componentCode.valueType == "Upload" ?
                                            (
                                              <Upload
                                                {...item.componentCode}
                                                name='file'
                                            ...
                                            (
                                              item.componentCode.valueType == 'Cascader' ?
                                                (
                                                  <Cascader
                                                    {...item.componentCode}
                                                    getPopupContainer={(triggerNode: any) => triggerNode.parentNode}
                                                ...
                                                (
                                                  item.componentCode.valueType == 'TextArea' ?
                                                    <TextArea
                                                      {...item.componentCode}
                                                      style={{ height: 120 }}
                                                      disabled={isDisabled ? isDisabled : (!!item.componentCode.disabled && item.componentCode.disabled ? item.componentCode.disabled : false)}
                                                    ...
                                                    :
                                                    (
                                                      item.componentCode.valueType == 'RichText' ?
                                                        // <RichText
                                                        ...
                                                        :
                                                        (
                                                          item.componentCode.valueType == 'ISelect' ?
                                                            <AutoComplete
                                                              {...item.componentCode}
                                                              getPopupContainer={(triggerNode: any) => triggerNode.parentNode}
                                                              filterOption={(input: any, option: any) =>
