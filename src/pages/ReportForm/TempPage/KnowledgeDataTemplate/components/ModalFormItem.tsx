...
const ModalFormItem: React.FC<{}> = (props: any) => {
  ...
  useEffect(() => {
    form.setFieldsValue({ ...props.recordDetial })
  ...
  const handleUploadChange = async ({ file, fileList }: any, name: any) => {
    ...
    if (file.response && file.response.code != 0) {
      message.error(file.response.message || '操作失败');
      return;
    }
                    ...
                    props.formFieldsProp.length > 0 ? props.formFieldsProp.filter((it: any) => it.isDisabled != 1).map((item: any, i: any) => (
                      <Col span={24} key={i}>
                        <Form.Item
                          ...
                          initialValue={(props.recordDetial && props.recordDetial[item.tableColum]) ? props.recordDetial[item.tableColum] : item.filterType == "datePicker" ? moment() : item.filterType == "select" && !item?.code && !!props.initData && props.initData[item.tableColum] && props.initData[item.tableColum].length ? (!!props.treeObj && props.treeObj[item.tableColum] ? props.treeObj[item.tableColum] : props.initData[item.tableColum][0][item.componentCode.fieldNames.value]) : undefined}
                        >
                          {
