...
const TableList: React.FC<{}> = (props: any) => {
    ...
    const onSearch = (e: any) => {
        queryFieldsProp.length && queryFieldsProp.forEach((item: any) => {
            // 时间范围
        ...
        getBizCodeData(intCode, { ...e, ...filterObj, ...treeObj }, pageSize, pageNum, 1);
    }
    ...
    const getFormItemValue = (record: any) => {
        if (record) {
            ...
            formFieldsProp.forEach((item: any) => {
                ...
                if (item.filterType == 'rangerPicker') {
                    if (!!newRowDetail[item.tableColum] && newRowDetail[item.tableColum]) {
                        // let arr: any = [moment(newRowDetail[item.dataIndex].split(',')[0]), moment(newRowDetail[item.dataIndex].split(',')[1])];
                        // newRowDetail[item.dataIndex] = arr;
                ...
                // if (item.filterType == 'select' && !!item.componentCode && item.componentCode?.mode) {
                //     if (!!newRowDetail[item.tableColum] && newRowDetail[item.tableColum]) {
                //         console.log('newRowDetail',newRowDetail);
                //         console.log('newRowDetail[item.tableColum]',newRowDetail[item.tableColum]);
                //         newRowDetail[item.tableColum] = newRowDetail[item.tableColum].split(',')
                //     }
                // }
            })
    ...
    const deleteRowData = async (record: any, code: any) => {
        ...
        if (res.response.code === 0) {
            message.success(res.response.message || '操作成功');
        } else {
            message.error(res.response.message || '操作失败');
        }
                                        ...
                                        item.type == 6 ?   //详情
                                            (
                                                <a key={i} onClick={() => {
                                                    ...
                                                    // setModalVisible(true);
                                                    // setModalTitle(item.name);
                                                    // setRecordDetial(getFormItemValue(rowData));
                                                    let rowData: any = getFormItemValue(record);
    ...
    return (
        <Card>
            <Card
                ...
                // className={[styles['typemgr-card'], 'ant-card-headborder',styles.tstitle].join(' ')}
                className={styles.tstableList}
                extra={
                    <React.Fragment>
                        <Space>
                            {
                                            ...
                                            ite.type == 2 ? //新增弹窗
                                                (
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            // 右侧列表新增
                                                            if (ite.link && ite.link.indexOf('/report-manage/ModalFormReport/page&id=') > -1) {
                                                                setIsDisabled(false);
                                                                setModalTitle(ite.name);
                                                                setModalPageLink(ite.link);
                                                                setModalFormTemplate(true)
                                                            } else if (ite.link && ite.link.indexOf('/report-manage/ModalFormProReport/page&id=') > -1) {
                                                                setIsDisabled(false);
                                                                setModalTitle(ite.name);
                                                                setModalPageLink(ite.link);
                                                                setModalFormProTemplate(true)
